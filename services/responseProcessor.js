const { findConversation } = require("./conversationFinder");
...
async function processConversationBuffer(conversationId, recruiterInstance) {
  const conversation = await findConversation(recruiterInstance, conversationId);
  ...
  if (!hasBufferExpired(conversation.lastMessageAt)) {
    return { processed: false, reason: "buffer_not_expired", waitMs: RESPONSE_BUFFER_MS };
  }

  const answer = conversation.pendingResponse;

  await updateConversation(conversation.conversationId, { processing: true });
  ...
  try {
    ...
    await updateConversation(conversation.conversationId, { processing: false });
    throw error;
  }
}
